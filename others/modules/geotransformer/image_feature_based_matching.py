...
from others.modules.ops import pairwise_distance


class ImageBasedMatching(nn.Module):
    def __init__(self, num_correspondences, dual_normalization=True):
        super(ImageBasedMatching, self).__init__()
        self.num_correspondences = num_correspondences
        self.dual_normalization = dual_normalization

    def forward(self, ref_img_feats, src_img_feats, ref_feats, src_feats, ref_masks=None, src_masks=None):
        ...
        ref_img_feats = ref_img_feats[ref_indices]
        src_img_feats = src_img_feats[src_indices]
        # select top-k proposals
        matching_scores = torch.exp(-pairwise_distance(ref_feats, src_feats, normalized=True))
        img_matching_scores = torch.exp(-pairwise_distance(ref_img_feats, src_img_feats, normalized=True))
        if self.dual_normalization:
            ...
            img_ref_matching_scores = img_matching_scores / img_matching_scores.sum(dim=1, keepdim=True)
            img_src_matching_scores = img_matching_scores / img_matching_scores.sum(dim=0, keepdim=True)
            img_matching_scores = img_ref_matching_scores * img_src_matching_scores
            matching_scores = torch.mul(matching_scores, img_matching_scores)
