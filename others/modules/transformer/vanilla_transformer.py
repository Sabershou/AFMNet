...
from others.modules.layers import build_dropout_layer
from others.modules.transformer.output_layer import AttentionOutput
...
class IFMultiHeadAttention(nn.Module):
    def __init__(self, d_model, num_heads, dropout=None):
        super(IFMultiHeadAttention, self).__init__()
        if d_model % num_heads != 0:
            raise ValueError('`d_model` ({}) must be a multiple of `num_heads` ({}).'.format(d_model, num_heads))
        ...
        self.proj_k = nn.Linear(self.d_model, self.d_model)
        self.proj_v = nn.Linear(self.d_model, self.d_model)
        # self.proj_iq = nn.Linear(self.d_model, self.d_model)
        # self.proj_ik = nn.Linear(self.d_model, self.d_model)
        # self.proj_iv = nn.Linear(self.d_model, self.d_model)

        self.dropout = build_dropout_layer(dropout)

    def forward(
        self, input_q, input_k, input_v, image_q, image_k, image_v, key_weights=None, key_masks=None, attention_factors=None, attention_masks=None
    ):
        """Vanilla Self-attention forward propagation.
        ...
        Args:
            ...
            input_k (Tensor): input tensor for key (B, M, C)
            input_v (Tensor): input tensor for value (B, M, C)
            image_q (Tensor): input tensor for query (B, N, C)
            image_k (Tensor): input tensor for key (B, M, C)
            image_v (Tensor): input tensor for value (B, M, C)
            key_weights (Tensor): soft masks for the keys (B, M)
            key_masks (BoolTensor): True if ignored, False if preserved (B, M)
            attention_factors (Tensor): factors for attention matrix (B, N, M)
            attention_masks (BoolTensor): True if ignored, False if preserved (B, N, M)
        ...
        k = rearrange(self.proj_k(input_k), 'b m (h c) -> b h m c', h=self.num_heads)
        v = rearrange(self.proj_v(input_v), 'b m (h c) -> b h m c', h=self.num_heads)
        # iq = rearrange(self.proj_iq(image_q), 'b n (h c) -> b h n c', h=self.num_heads)
        # ik = rearrange(self.proj_ik(image_k), 'b m (h c) -> b h m c', h=self.num_heads)
        # iv = rearrange(self.proj_iv(image_v), 'b m (h c) -> b h m c', h=self.num_heads)

        attention_scores = torch.einsum('bhnc,bhmc->bhnm', q, k) / self.d_model_per_head ** 0.5
        # attention_scores_image = torch.einsum('bhnc,bhmc->bhnm', iq, ik) / self.d_model_per_head ** 0.5
        if attention_factors is not None:
            attention_scores = attention_factors.unsqueeze(1) * attention_scores
        if key_weights is not None:
            attention_scores = attention_scores * key_weights.unsqueeze(1).unsqueeze(1)
        ...
        if attention_masks is not None:
            attention_scores = attention_scores.masked_fill(attention_masks, float('-inf'))
        attention_scores = F.softmax(attention_scores, dim=-1)
        # attention_scores = F.softmax(attention_scores_image, dim=-1)
        attention_scores = self.dropout(attention_scores)
        # attention_scores = self.dropout(attention_scores_image)

        hidden_states_points = torch.matmul(attention_scores, v)
        # hidden_states_image = torch.matmul(attention_scores_image, iv)
        # hidden_states = hidden_states_image + hidden_states_points
        hidden_states = hidden_states_points
...
class IFAttentionLayer(nn.Module):
    def __init__(self, d_model, num_heads, dropout=None):
        super(IFAttentionLayer, self).__init__()
        self.attention = IFMultiHeadAttention(d_model, num_heads, dropout=dropout)
        self.linear = nn.Linear(d_model, d_model)
        self.dropout = build_dropout_layer(dropout)
        self.norm = nn.LayerNorm(d_model)
    ...
    def forward(
        ...
        hidden_states, attention_scores_points = self.attention(
            input_states,
            memory_states,
            memory_states,
            input_image_states,
        ...
        hidden_states = self.dropout(hidden_states)
        output_states = self.norm(hidden_states + input_states)
        return output_states, attention_scores_points
...
class IFTransformerLayer(nn.Module):
    def __init__(self, d_model, num_heads, dropout=None, activation_fn='ReLU'):
        super(IFTransformerLayer, self).__init__()
        self.attention = IFAttentionLayer(d_model, num_heads, dropout=dropout)
        self.output = AttentionOutput(d_model, dropout=dropout, activation_fn=activation_fn)
