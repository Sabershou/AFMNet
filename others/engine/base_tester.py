...
from others.utils.torch import initialize
from others.engine.logger import Logger
...
def inject_default_parser(parser=None):
    ...
    parser.add_argument('--test_epoch', type=int, default=40, help='test epoch')
...
class BaseTester(abc.ABC):
    def __init__(self, cfg, parser=None, cudnn_deterministic=True):
        ...
        # logger
        log_file = osp.join(cfg.log_dir, 'test-{}.log'.format(self.args.test_epoch))
        # log_file = osp.join(cfg.log_dir, 'test-{}-{}.log'.format(self.args.test_epoch, self.args.benchmark))
