...
import numpy as np
import torch
from others.utils.img_utils import point2cam
from others.modules.ops import grid_subsample, radius_search
from others.utils.torch import build_dataloader
...
def registration_collate_fn_stack_mode(
    ...
    if precompute_data:
        ...
        collated_dict['lengths'] = lengths
    collated_dict['batch_size'] = batch_size
    length_f = collated_dict['lengths'][1][0]
    length_c = collated_dict['lengths'][3][0]
    ref_x_scale, ref_y_scale = collated_dict['ref_x_scale'], collated_dict['ref_y_scale']
    src_x_scale, src_y_scale = collated_dict['src_x_scale'], collated_dict['src_y_scale']
    ref_img, src_img = collated_dict['ref_img'], collated_dict['src_img']
    # img_pts_2, keep_idx_2 = point2cam(collated_dict['points'][-2], collated_dict['intrinsic'], 640, 480)
    ref_img_pts_0, ref_keep_idx_0 = point2cam(points_list[0], collated_dict['ref_intrinsic'],
                                      ref_img.shape[1], ref_img.shape[0], ref_x_scale, ref_y_scale)
    src_img_pts_0, src_keep_idx_0 = point2cam(points_list[1], collated_dict['src_intrinsic'],
                                      src_img.shape[1], src_img.shape[0], src_x_scale, src_y_scale)
    ref_img_pts_1, ref_keep_idx_1 = point2cam(collated_dict['points'][1][:length_f], collated_dict['ref_intrinsic'],
                                      ref_img.shape[1], ref_img.shape[0], ref_x_scale, ref_y_scale)
    src_img_pts_1, src_keep_idx_1 = point2cam(collated_dict['points'][1][length_f:], collated_dict['src_intrinsic'],
                                      src_img.shape[1], src_img.shape[0], src_x_scale, src_y_scale)
    ref_img_pts_4, ref_keep_idx_4 = point2cam(collated_dict['points'][-1][:length_c], collated_dict['ref_intrinsic'],
                                      ref_img.shape[1], ref_img.shape[0], ref_x_scale, ref_y_scale)
    src_img_pts_4, src_keep_idx_4 = point2cam(collated_dict['points'][-1][length_c:], collated_dict['src_intrinsic'],
                                      src_img.shape[1], src_img.shape[0], src_x_scale, src_y_scale)

    collated_dict['ref_img_pts_4'] = ref_img_pts_4.type(torch.int32)
    collated_dict['ref_keep_idx_4'] = ref_keep_idx_4
    collated_dict['src_img_pts_4'] = src_img_pts_4.type(torch.int32)
    collated_dict['src_keep_idx_4'] = src_keep_idx_4
    collated_dict['ref_img_pts_1'] = ref_img_pts_1.type(torch.int32)
    collated_dict['ref_keep_idx_1'] = ref_keep_idx_1
    collated_dict['src_img_pts_1'] = src_img_pts_1.type(torch.int32)
    collated_dict['src_keep_idx_1'] = src_keep_idx_1
    collated_dict['ref_img_pts_0'] = ref_img_pts_0.type(torch.int32)
    collated_dict['ref_keep_idx_0'] = ref_keep_idx_0
    collated_dict['src_img_pts_0'] = src_img_pts_0.type(torch.int32)
    collated_dict['src_keep_idx_0'] = src_keep_idx_0

    return collated_dict


def registration_collate_fn_stack_mode_kitti(
    data_dicts, num_stages, voxel_size, search_radius, neighbor_limits, precompute_data=True
):
    r"""Collate function for registration in stack mode.
    ...
    collated_dict['batch_size'] = batch_size
    length_f = collated_dict['lengths'][1][0]
    length_c = collated_dict['lengths'][-1][0]
    ref_x_scale, ref_y_scale = collated_dict['ref_x_scale'], collated_dict['ref_y_scale']
    src_x_scale, src_y_scale = collated_dict['src_x_scale'], collated_dict['src_y_scale']
    ref_img, src_img = collated_dict['ref_img'], collated_dict['src_img']
    ref_img_pts_1, ref_keep_idx_1 = point2cam(collated_dict['points'][1][:length_f], collated_dict['ref_proj_matrix'],
                                      ref_img.shape[1], ref_img.shape[0], ref_x_scale, ref_y_scale)
    src_img_pts_1, src_keep_idx_1 = point2cam(collated_dict['points'][1][length_f:], collated_dict['src_proj_matrix'],
                                      src_img.shape[1], src_img.shape[0], src_x_scale, src_y_scale)
    ref_img_pts_4, ref_keep_idx_4 = point2cam(collated_dict['points'][-1][:length_c], collated_dict['ref_proj_matrix'],
                                      ref_img.shape[1], ref_img.shape[0], ref_x_scale, ref_y_scale)
    src_img_pts_4, src_keep_idx_4 = point2cam(collated_dict['points'][-1][length_c:], collated_dict['src_proj_matrix'],
                                      src_img.shape[1], src_img.shape[0], src_x_scale, src_y_scale)
