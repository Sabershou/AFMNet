...
class ResNetFCN(nn.Module):
    def __init__(self, backbone="resnet34", pretrained=True, config=None):
        ...
        self.hiden_size = config.hidden_size
    ...
    def forward(self, data_dict, img):
        x = img
        x = torch.unsqueeze(x, dim=0).permute(0, 3, 1, 2).float()
        ...
        layer1_in = self.layer1(self.maxpool(conv1_out))
        layer2_in = self.layer2(layer1_in)
        layer3_in = self.layer3(layer2_in)
        # layer4_out = self.layer4(layer3_out)
        # img_encoder = layer4_out

        # Deconv
        layer1_out = self.deconv_layer1(layer1_in)
        layer2_out = self.deconv_layer2(layer2_in)
        layer3_out = self.deconv_layer3(layer3_in)
        # layer4_out = self.deconv_layer4(layer4_out)
        img_encoder_1 = layer1_out
        img_encoder_3 = torch.cat([layer3_out, layer1_out], dim=1)
        ...
        # y = torch.cat((layer1_out, layer2_out, layer3_out, layer4_out), dim=1)
        # y = self.linear(y)

        return img_encoder_1, img_encoder_3
