...
from others.engine import Logger
from others.modules.registration import weighted_procrustes
from others.utils.summary_board import SummaryBoard
from others.modules.ops.transformation import apply_transform
from others.utils.open3d import registration_with_ransac_from_correspondences
from others.utils.registration import (
    evaluate_sparse_correspondences,
    evaluate_correspondences,
    compute_registration_error,
)
from others.datasets.registration.threedmatch.utils import (
    get_num_fragments,
    get_scene_abbr,
...
from others.utils.common import ensure_dir


def make_parser():
    parser = argparse.ArgumentParser()
    parser.add_argument('--test_epoch', default=None, type=int, help='test epoch')
    parser.add_argument('--benchmark', choices=['3DMatch', '3DLoMatch'], default='3DMatch', required=True,
                        help='test benchmark')
    parser.add_argument('--method', default='ransac', choices=['lgr', 'ransac', 'svd'], required=True,
                        help='registration method')
    parser.add_argument('--num_corr', type=int, default=1000, help='number of correspondences for registration')
...
def eval_one_epoch(args, cfg, logger):
    ...
    num_corr = str(args.num_corr)
    failed_case = osp.join(cfg.output_dir, 'failed case')
    ensure_dir(failed_case)
    ...
    for scene_root in scene_roots:
        ...
        failed_scene = osp.join(failed_case, args.method, num_corr, args.benchmark, scene_name)
        ensure_dir(failed_scene)

        for file_name in file_names:
            ref_frame, src_frame = [int(x) for x in osp.basename(file_name).split('.')[0].split('_')]

            data_dict = np.load(file_name)
            numpy_file = osp.basename(file_name)
            ...
            # distance_threshold = 0.05  # 内点到平面模型的最大距离
            ...
            # clouds = []
            # while number > 1000:
            #     _, inliers = out.segment_plane(distance_threshold, ransac_n, num_iterations)
            #     distance_threshold *= 0.9
            #     inlier_cloud = out.select_down_sample(inliers)
            #     if len(inliers) > 500:
            #         clouds.append(inlier_cloud)
            #     out = out.select_down_sample(inliers, invert=True)
            ...
            if gt_indices[ref_frame, src_frame] != -1:
                ...
                # transform_eval = torch.from_numpy(transform)
                # estimated_transform_eval = torch.from_numpy(estimated_transform)
                ...
                # realignment_transform = torch.matmul(torch.inverse(transform_eval), estimated_transform_eval)
                # realigned_src_points_f = apply_transform(src_points, realignment_transform)
                # error = torch.linalg.norm(realigned_src_points_f - src_points, dim=1).mean()
                ...
                if accepted:
                    ...
                    message += ', r_RTE: {:.4f}'.format(rte)
                # else:
                #     numpy_path = osp.join(failed_scene, numpy_file)
                #     np.savez_compressed(numpy_path,
                #                         ref_points_c=data_dict['ref_points_c'],
                #                         src_points_c=data_dict['src_points_c'],
                #                         ref_node_corr_indices=data_dict['ref_node_corr_indices'],
                #                         src_node_corr_indices=data_dict['src_node_corr_indices'],
        ...
        message = '  Correspondence: {}'.format((args.num_corr))
        message += ', c_PIR: {:.3f}'.format(coarse_precision)
        ...
        message += ', RR: {:.4f}'.format(recall)
        message += ', mean_RRE: {:.3f}'.format(mean_rre)
        message += ', mean_RTE: {:.4f}'.format(mean_rte)
        message += ', median_RRE: {:.3f}'.format(median_rre)
        message += ', median_RTE: {:.4f}'.format(median_rte)
        logger.info(message)
    ...
    message += ', mean_RTE: {:.4f}'.format(registration_meter.mean('mean_rte'))
    message += ', median_RRE: {:.3f}'.format(registration_meter.mean('median_rre'))
    message += ', median_RTE: {:.4f}'.format(registration_meter.mean('median_rte'))
    logger.critical(message)
